...
let selectedTable = null; // Mesa seleccionada para mostrar controles de rotación
let isDragging = false; // Para distinguir entre clic y arrastre
...
function init() {
    ...
    floorPlan.addEventListener('click', (e) => {
        if (e.target === floorPlan || e.target.classList.contains('floor-plan-svg')) {
    ...
    if (!tables.find(t => t.id === 'geschenke-table')) {
        const geschenkeTable = {
            id: 'geschenke-table',
            x: 100, // Dentro de la sala (entre 20 y 919)
            y: 20,  // Pegada a la pared superior (dentro de la sala)
            seats: 8,
            isRoyal: false,
            isGeschenke: true,
...
function rotateTable(id) {
    ...
    if (table) {
        ...
        saveTables();
        renderTables();
        // Mantener los controles de rotación visibles después de rotar
        setTimeout(() => {
...
function showRotationControls(tableId) {
    ...
    // Botón para rotar 45° en sentido horario
    ...
    // Posicionar los controles cerca de la mesa
    controlsDiv.style.left = `${table.x + 70}px`; // A la derecha de la mesa
...
function renderTables() {
    ...
    // Ocultar controles de rotación al renderizar
    hideRotationControls();
...
function handleMouseUp(e) {
    if (draggedTable) {
        ...
        // Si no se arrastró (solo se hizo clic), mostrar controles de rotación
        if (!isDragging) {
